        ...
        for i in range(N):
            ...
            if mask.size > 0:
                ...
                if contours:
                    ...
                    depth = self.depth_image[int(rect[0][1]), int(rect[0][0])] / 1000.0  # Assuming depth is in mm
                    real_x = (rect[0][0] - cx) * depth / fx
                    real_y = (rect[0][1] - cy) * depth / fy
                    real_z = depth

                    # 객체 정보 표시
                    label_map = {1: "Bottle", 2: "Cup", 12: "Stop Sign"}
                    label = f"{label_map[class_id]}: {scores[i]:.2f}, X: {real_x:.2f}, Y: {real_y:.2f}, Z: {real_z:.2f}"
                    cv2.putText(image, label, (int(rect[0][0]), int(rect[0][1]) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)

        # 결과 이미지 표시
        cv2.imshow("Detected Objects with Rotated Boxes", image)
        cv2.waitKey(3)
